...
public class MainProgram extends javax.swing.JFrame {
    ...
    private JButton[][] facelet = new JButton[6][16];
    private final JButton[] colorSel = new JButton[6];
    private final int FSIZE = 35;
    private final int[] XOFF = { 4, 8, 4, 4, 0, 12 };// Offsets for facelet display
    private final int[] YOFF = { 0, 4, 4, 8, 4, 4 };
    ...
    private void initGUI() {
        ...
        for (int i = 0; i < 6; i++)
            for (int j = 0; j < 16; j++) {
                ...
                facelet[i][j].setBounds(FSIZE * XOFF[i] + FSIZE * (j % 4), FSIZE * YOFF[i] + FSIZE * (j / 4), FSIZE, FSIZE);
    ...
    private void solveCube(ActionEvent evt) {
        ...
        for (int i = 0; i < 6; i++)
            // read the 96 facelets
            for (int j = 0; j < 16; j++) {
                if (facelet[i][j].getBackground() == COLORS[0])
                    s.setCharAt(16 * i + j, 'U');
                if (facelet[i][j].getBackground() == COLORS[1])
                    s.setCharAt(16 * i + j, 'R');
                if (facelet[i][j].getBackground() == COLORS[2])
                    s.setCharAt(16 * i + j, 'F');
                if (facelet[i][j].getBackground() == COLORS[3])
                    s.setCharAt(16 * i + j, 'D');
                if (facelet[i][j].getBackground() == COLORS[4])
                    s.setCharAt(16 * i + j, 'L');
                if (facelet[i][j].getBackground() == COLORS[5])
                    s.setCharAt(16 * i + j, 'B');
            }
